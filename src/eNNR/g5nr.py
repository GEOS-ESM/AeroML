...
#     print(pi)
    ...
    if "SensorZenith" in self.variables:
        ...
        self.variables += [ 'GlintAngle', 'ScatteringAngle' ]
...
class TARGET(G5NR):
    ...
    def __init__(self,Files=None,GlintTresh=40.):
        ...
        G5NR.__init__(self,Files=Files, Names=Target)
        # self.reduce(self.filter(GlintTresh=GlintTresh))

    def filter(self,GlintTresh=40.):

        iValid = np.ones(self.nobs,dtype=bool)
...
class FEATURES(G5NR):
    ...
    """
    def __init__(self,Files,features,GlintThresh=40.):
        """
        Read files and optionally provide Q/C.
        """
        if Files is None:
    ...
    def filter(self,GlintThresh=40.):
        """
