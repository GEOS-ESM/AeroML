...
import os, sys
from   pyabc.abc_viirs         import ABC_DT_Land
import argparse
...
if __name__ == "__main__":
    ...
    # -----------------------------------
    if args.test:
        outdir = 'test_dtland_plots'
    else:
        outdir = 'val_dtland_plots'
    os.makedirs(outdir,exist_ok=True)

    invars = deep.comblist[0]
    netFile = deep.outdir+"/"+".".join(invars)+'_Tau.net'


    # load the net
    deep.net = deep.loadnet(netFile)
    deep.Input = deep.comblist[0]

    # set I
    I = deep.iTest

    # Get the target, original data, and NN predicted data
    targets  = deep.getTargets(I,noscale=True)
    results = deep.eval(I,noscale=True).squeeze()
    name = 'm'+deep.Target[0][1:]
    original = np.log(deep.__dict__[name][I] + 0.01)

    figfile = outdir + '/kde2d_dtland_std.png'
    _plot2dKDE(targets,original,x_bins=None,y_bins=None,
             x_label='AERONET', y_label='STANDARD Retrieval',figfile=figfile,title='DT-LAND 550 nm AOD')

    figfile = outdir + '/kde2d_dtland_nnr.png'
    _plot2dKDE(targets,results,x_bins=None,y_bins=None,
             x_label='AERONET', y_label='NNR Retrieval',figfile=figfile,title='DT-LAND 550 nm AOD')
    ...
    if (outliers > 0) and (K is None):
        ...
        # load the net
        deep_out.net = deep.net
        deep_out.Input = deep.Input

        # set I
        I = deep_out.iTest

        # Get the target, original data, and NN predicted data
        targets  = deep_out.getTargets(I,noscale=True)
        results = deep_out.eval(I,noscale=True).squeeze()
        name = 'm'+deep_out.Target[0][1:]
        original = np.log(deep_out.__dict__[name][I] + 0.01)

        figfile = outdir + '/kde2d_dtland_outliers_std.png'
        _plot2dKDE(targets,original,x_bins=None,y_bins=None,
             x_label='AERONET', y_label='STANDARD Retrieval',figfile=figfile,title='DT-LAND 550 nm AOD')

        figfile = outdir + '/kde2d_dtland_outliers_nnr.png'
        _plot2dKDE(targets,results,x_bins=None,y_bins=None,
             x_label='AERONET', y_label='NNR Retrieval',figfile=figfile,title='DT-LAND 550 nm AOD')      
