...
class GIANT(object):
  ...
  def __init__ (self,filename,xVars=(),only_good=True,tymemax=None):
    ...
    # Form python time
    # ----------------
    T = self.ISO_DateTime[:,0:18].data
    # Bug in dataset, first field is blank, so discard first record
    T[0] = T[1]
    self.aTau550[0] = -9999.0
    self.tyme = array([ isoparse(t.tobytes().decode('utf-8')) for t in T ])
    ...
    del self.ISO_DateTime
    self.variables.remove('ISO_DateTime')
    self.variables.append('tyme')
